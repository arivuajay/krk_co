...
<table  border="0" cellspacing="0" class="table table-bordered table-striped table_order" id="list-table" cellpadding="0">
	...
	<tr>				 
		...
		<td><?php echo $sale->orderdetail->total_order_value; ?></td>
		<td><?php echo (!empty($sale->invoicedAmt)) ? Myclass::GetSiteSetting("AMOUNT_FORMAT")." ".$sale->invoicedAmt : "--"; ?></td>
		<td><?php
		    echo CHtml::link('<i class="icon-pencil"></i>',array('/production/ship/view','id'=>$sale->so_id),array('title'=>'View','rel'=>'tooltip'));
		    echo CHtml::link('<i class="icon-trash"></i>',array('/production/pick/delete','id'=>$sale->so_id),array('onclick'=>"return confirm('Are you sure you want to delete?')",'title'=>'Delete','rel'=>'tooltip')); 
		?>	
		</td>
	</tr>
	<?php $i++;	endforeach; ?>
